...
def proceseaza_litera(stari_curente,litera):
    global arce,alfabet
    if litera not in alfabet:   #verifica daca litera se gaseste in alfabet (util in cazul unu nfa doar cu lamda miscari)
        return []
    else:
        new_stari=[]    #fac o multime noua de stari pentru a evita stergerea din lista stari_curente in timp ce iterez peste ea
        for stare_de_proc in stari_curente:
            am_stari = []
            for tran in arce:
...
def evaluate(cuvant):
    global F
    stari_curente =[q0]
    stari_curente = proceseaza_lamda(stari_curente) #proceseaza toate lamda-miscarile posibile de la starea initiala
    print("{}|-{}".format(stari_curente,cuvant))
